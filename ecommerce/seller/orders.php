...
$sql = "SELECT order_id,product_name,product_price,product_image_path,order_date,order_status,username FROM myorder inner join product on myorder.product_id=product.product_id inner join user on myorder.user_id=user.user_id where myorder.product_id in (select product_id from product where seller_id=$sidint) order by order_id";
...
<div class='ms-2 me-auto'>
    ...
    <form action='changestatus.php' method='post'>
        <label for='text'>Change status: </label>
        <input type='text' name='status'  label='Change status' required>
